document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderDashboard(key, role) {
        ...
        if (uniqueBlockedOrders > 0) {
            statOrdersSub.innerHTML = `Se tiene <span style="font-weight:700;">${uniqueBlockedOrders}</span> para gestionar liberación! (Ped. Bloqueados)`;
        ...
        if (rejectedClients.size > 0) {
            clientSubHtml += `<div style="color: #B91C1C;">Se tiene <span style="font-weight:700;">${rejectedClients.size}</span> para reforzar recepción de pedido. (Rechazos)</div>`;
        }
...
function renderGlobalView() {
    ...
    // Capacidad Total de toda la gerencia
    const totalHLCapacity = Object.values(CAPACITIES).reduce((acc, curr) => acc + curr.hl, 0);
    ...
    let html = `
        ...
        <div class="glass-panel" style="background: var(--bg-secondary); border: 1px solid var(--glass-border); border-radius: 16px; overflow: hidden; box-shadow: 0 10px 30px rgba(0,0,0,0.05); margin-bottom: 2rem; padding: 1.5rem;">
            <h3 style="margin-top: 0; color: var(--text-primary); font-size: 1.2rem; margin-bottom: 1rem;"><i class="ph ph-trend-up"></i> Proyección de Carga (HL) - Próximos 7 Días</h3>
                ...
                <table style="width: 100%; border-collapse: collapse; text-align: left;">
                    <thead>
                        <tr style="background: rgba(0,0,0,0.02); border-bottom: 2px solid var(--glass-border);">
                            ...
                            <th style="padding: 1.2rem; color: var(--text-secondary); font-weight: 700; text-transform: uppercase; font-size: 0.85rem; letter-spacing: 1px; text-align: right;">HL (Cap: ${totalHLCapacity})</th>
                    ...
                    <tbody>
                        ${sortedDates.map((date, idx) => {
                            ...
                            const capPct = (data.hl / totalHLCapacity) * 100;
                    ...
                    datasets: [
                        {
                            label: 'Proyección HL',
                            data: chartData,
                            backgroundColor: chartData.map(val => val > totalHLCapacity ? 'rgba(220, 38, 38, 0.8)' : 'rgba(255, 153, 0, 0.8)'), // Rojo si sobrepasa la capacidad total
                            borderRadius: 6,
                            borderWidth: 0
                        },
                        {
                            label: 'Capacidad Máxima',
                            data: sortedDates.map(() => totalHLCapacity),
                            type: 'line',
                            borderColor: '#374151',
                            borderWidth: 2,
                            borderDash: [5, 5],
