document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadDataAutomatically() {
        ...
        try {
            ...
            // Verificar si el archivo comienza con 'PK' (firma de archivos ZIP/XLSX)
            // Si comienza con '<!', es probable que el servidor esté devolviendo un HTML (ej. un 404 disfrazado de 200)
            if (data.length > 2 && data[0] === 60 && data[1] === 33) { // '<!'
                alert("AVISO CRÍTICO: El archivo 'pedidos.xlsx' que se descargó no es un Excel, ¡es una página web! Esto significa que el archivo no está subido correctamente a GitHub o el enlace está roto. Sube tu archivo pedidos.xlsx a GitHub otra vez.");
                return;
            }
            if (data.length > 2 && data[0] !== 80 && data[1] !== 75) { // 'PK'
                alert("AVISO: El archivo descargado no tiene el formato correcto de Excel (.xlsx). Podría estar corrupto o guardado como otro tipo de archivo.");
            }
