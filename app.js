document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderDashboard(key, role) {
        ...
        if (statPrepago) {
            statPrepago.textContent = prepagoClients.size;
        }
        
        const statN3 = document.getElementById('stat-n3');
        if (statN3) {
            statN3.textContent = n3Clients.size;
        }
        
        if (statSales) {
            // Updated for the new big card format in stats-grid
            statSales.innerHTML = `${cajasTotal.toLocaleString('en-US', { maximumFractionDigits: 2 })} <span style="font-size: 0.6em; color: #6B7280; font-weight: 500;">CA</span><br>${hlTotal.toLocaleString('en-US', { maximumFractionDigits: 2 })} <span style="font-size: 0.6em; color: #6B7280; font-weight: 500;">HL</span>`;
        }
