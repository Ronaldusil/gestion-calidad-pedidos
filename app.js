document.addEventListener('DOMContentLoaded', () => {
    ...
    function processExcelData(data) {
        // Limpiar espacios en los nombres de las columnas (ej: SAP puede exportar "ZV " en vez de "ZV")
        ...
        if (globalData.length === 0 && cleanData.length > 0) {
            alert("AVISO: El Excel cargó correctamente, pero NINGUNA fila cumple con las reglas (TpMt = FERT y ClVt = ZPP3/ZPP5/ZPPV).");
        }
        ...
        if (currentUserRole === 'seller') {
            ...
            if (!sellersData[currentUserZV] || sellersData[currentUserZV].length === 0) {
                alert("AVISO: No hay ningún pedido (con FERT y ZPP) asignado a tu código de vendedor (" + currentUserZV + ") en este Excel.");
            }
        }
