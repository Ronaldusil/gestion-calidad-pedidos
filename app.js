...
document.addEventListener('DOMContentLoaded', () => {
  if (window.Chart && window.ChartDataLabels) Chart.register(ChartDataLabels);
    ...
    function renderDashboard(key, role) {
        ...
        if (statCategories) {
            statCategories.style.display = 'grid';
            statCategories.style.gridTemplateColumns = '1fr 1fr';
            statCategories.style.columnGap = '24px';
            statCategories.style.rowGap = '10px';
            
            let catHtml = '';
            Object.keys(categoryStats).sort((a,b) => categoryStats[b].cajas - categoryStats[a].cajas).forEach(cat => {
                catHtml += `<div style="display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid #f8fafc; padding-bottom:4px;">
                    <span style="font-size:0.85rem; color:#475569; font-weight: 500;">${cat}</span>
                    <strong style="font-size:0.9rem; color:#0f172a;">${categoryStats[cat].cajas.toLocaleString('en-US', {maximumFractionDigits:0})} <span style="font-size:0.75em;color:var(--success);">CA</span></strong>
                </div>`;
            });
        ...
        paginatedOrders.forEach((order, index) => {
            ...
            const valorNetoVal = parseFloat(order["ValorNeto"]) || 0;
            const impuestoVal = parseFloat(order["Importe del impuesto"]) || 0;
            const valorNeto = valorNetoVal + impuestoVal;
            ...
            tr.innerHTML = `
                ...
                <td class="celda-numero">
                    <strong>${caja}</strong> <span style="font-size: 0.8em; color: var(--success);">CA</span>
                </td>
                <td class="celda-numero">
                    <strong>${hl.toFixed(2)}</strong> <span style="font-size: 0.8em; color: var(--accent-primary);">HL</span>
                </td>
                ...
                <td>
                    ...
                    <div class="date-badge" class="badge ${tipoPagoStr.toLowerCase().includes('contado') ? 'badge-contado' : 'badge-credito'}">${tipoPagoStr}</div>
