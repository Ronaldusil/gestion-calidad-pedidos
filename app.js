...
function renderGlobalView() {
    ...
    // Calcular totales globales ("Total Gerencia")
    ...
    globalOrders.forEach(o => {
        ...
        evolution[dateStr].orders.add(o["Nº pedido cliente"]);
        const qty = parseFloat(o["Cantidad de pedido"]) || 0;
        evolution[dateStr].cajas += qty;
        if (String(o["AP"] || "").trim().toUpperCase() === "C") {
            evolution[dateStr].blocked.add(o["Nº pedido cliente"]);
        }
    });
    
    const sortedDates = Object.keys(evolution).sort((a, b) => b.localeCompare(a)); // Descendente

    let html = `
        <h2 style="font-size: 2rem; color: var(--text-primary); margin-bottom: 2rem; text-align: center; font-weight: 700;">Resumen Ejecutivo</h2>
        
        <!-- Tarjeta Total Gerencia -->
        <div class="stat-card glass-panel" style="flex-direction: column; align-items: flex-start; gap: 1rem; margin-bottom: 2rem; background: var(--bg-secondary); border: 1px solid var(--glass-border); width: 100%;">
            <div style="display: flex; align-items: center; gap: 1rem; width: 100%; border-bottom: 1px solid var(--glass-border); padding-bottom: 1rem;">
                <div class="stat-icon-wrapper" style="background: rgba(255, 87, 34, 0.15); color: var(--accent-primary);">
                    <i class="ph ph-chart-line-up"></i>
                </div>
                <h3 style="font-size: 1.8rem; color: var(--text-primary); margin: 0;">Total Gerencia (Evolución)</h3>
            </div>
            
            <div style="width: 100%; overflow-x: auto; margin-top: 0.5rem;">
                <table style="width: 100%; border-collapse: collapse; text-align: left;">
                    <thead>
                        <tr style="border-bottom: 2px solid var(--glass-border);">
                            <th style="padding: 1rem; color: var(--text-secondary);">Fecha de Entrega</th>
                            <th style="padding: 1rem; color: var(--text-secondary);">Pedidos Totales</th>
                            <th style="padding: 1rem; color: var(--text-secondary);">Volumen (Cajas/MKP)</th>
                            <th style="padding: 1rem; color: var(--text-secondary);">Bloqueados</th>
                        </tr>
                    </thead>
                    <tbody>
                        ${sortedDates.map(date => `
                            <tr style="border-bottom: 1px solid var(--glass-border);">
                                <td style="padding: 1rem; color: var(--text-primary); font-weight: 600;">${date}</td>
                                <td style="padding: 1rem; color: var(--text-primary);">${evolution[date].orders.size}</td>
                                <td style="padding: 1rem; color: var(--success); font-weight: 600;">${evolution[date].cajas.toLocaleString('en-US', { maximumFractionDigits: 2 })}</td>
                                <td style="padding: 1rem; color: var(--danger); font-weight: 600;">${evolution[date].blocked.size}</td>
                            </tr>
                        `).join('')}
                    </tbody>
                </table>
        ...
        <!-- Tarjetas por CD -->
        <h3 style="font-size: 1.5rem; color: var(--text-secondary); margin-bottom: 1rem; font-weight: 600;">Por Centro de Distribución</h3>
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 1.5rem;">
    `;
    
    cds.forEach(cd => {
        const orders = cdsData[cd] || [];
        
        // Stats
        const uniqueOrders = new Set(orders.map(o => o["Nº pedido cliente"])).size;
        const bloqueados = orders.filter(o => String(o["AP"] || "").trim().toUpperCase() === "C");
        const uniqueBlocked = new Set(bloqueados.map(o => o["Nº pedido cliente"])).size;
        ...
        // Colores y logos basados en el CD para darles vida (ejemplo: estilos dinámicos)
        let iconHtml = '<i class="ph ph-buildings"></i>';
        
        html += `
            <div class="stat-card glass-panel" style="flex-direction: column; align-items: flex-start; gap: 1rem; border-top: 4px solid var(--accent-primary);">
                <div style="display: flex; align-items: center; gap: 1rem; width: 100%; border-bottom: 1px solid rgba(0,0,0,0.05); padding-bottom: 1rem;">
                    <div class="stat-icon-wrapper" style="background: rgba(255, 87, 34, 0.15); color: var(--accent-primary);">
                        ${iconHtml}
                    </div>
                    <h3 style="font-size: 1.5rem; color: var(--text-primary); margin: 0;">${cd}</h3>
                </div>
                
                <div style="width: 100%; display: flex; justify-content: space-between; margin-top: 0.5rem;">
                    <span style="color: var(--text-secondary);">Pedidos Totales:</span>
                    <strong style="color: var(--text-primary); font-size: 1.2rem;">${uniqueOrders}</strong>
                </div>
                
                <div style="width: 100%; display: flex; justify-content: space-between;">
                    <span style="color: var(--text-secondary);">Volumen (Cajas/MKP):</span>
                    <strong style="color: var(--success); font-size: 1.2rem;">${totalCajas.toLocaleString('en-US', { maximumFractionDigits: 2 })}</strong>
                </div>
                
                <div style="width: 100%; display: flex; justify-content: space-between;">
                    <span style="color: var(--text-secondary);">Pedidos Bloqueados:</span>
                    <strong style="color: var(--danger); font-size: 1.2rem;">${uniqueBlocked}</strong>
                </div>
                
                <button class="btn-primary" style="width: 100%; margin-top: 1rem; justify-content: center; background: rgba(0,0,0,0.05); color: var(--text-primary);" onclick="selectCD('${cd}')" onmouseover="this.style.background='var(--accent-primary)'; this.style.color='#fff';" onmouseout="this.style.background='rgba(0,0,0,0.05)'; this.style.color='var(--text-primary)';">
