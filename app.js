...
document.addEventListener('DOMContentLoaded', () => {
  if (window.Chart && window.ChartDataLabels) Chart.register(ChartDataLabels);
    ...
    function renderDashboard(key, role) {
        ...
        if (thZv) thZv.style.display = role === 'admin' ? '' : 'none';
            ...
            tr.innerHTML = `
                ...
                <td style="white-space: nowrap; text-align: right; font-variant-numeric: tabular-nums;">
                    ${valorNeto >= 2000 ? 
                        `<div style="background: #FEF3C7; color: #92400E; border: 1px solid #FCD34D; padding: 4px 8px; border-radius: 6px; display: inline-block; text-align: center; box-shadow: 0 1px 2px rgba(0,0,0,0.05);" title="Requiere voucher">
                            ...
                            <div style="font-size: 0.65rem; text-transform: uppercase; letter-spacing: 0.5px; font-weight: 700; margin-top: 2px; color: #B45309;">ÔÜá´©Å Requiere Voucher</div>
                        </div>` 
                        : 
                ...
                <td style="text-align: center;">
                    ${tipoPagoStr.toLowerCase().includes('contado') 
                        ? `<div class="date-badge" style="background: #E0F2FE; color: #0369A1; border: 1px solid #BAE6FD; padding: 4px 8px; font-weight: 600; font-size: 0.8rem; display: inline-flex; align-items: center; gap: 4px;">­ƒÆÁ ${tipoPagoStr}</div>`
                        : `<div class="date-badge" style="background: #F1F5F9; color: #475569; border: 1px solid #E2E8F0; padding: 4px 8px; font-weight: 600; font-size: 0.8rem; display: inline-flex; align-items: center; gap: 4px;">­ƒÆ│ ${tipoPagoStr}</div>`
                    }
                ...
                <td style="text-align: center;">
                    <button class="btn-detalle" onclick="toggleDetails('det-${orderNum.replace(/\W/g, '')}', this)" title="Ver Detalle" style="background:none; border:none; cursor:pointer; font-size:1.2rem; transition: transform 0.2s;">Ô×ò</button>
                </td>
                <td class="text-right">
