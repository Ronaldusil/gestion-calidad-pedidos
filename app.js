document.addEventListener('DOMContentLoaded', () => {
    ...
    function processExcelData(workbook, rawBytes) {
        ...
        if (headerRowIndex === -1 || bestMatchCount === 0) {
            // Solo mostramos alerta si el usuario subió un archivo local, para no asustarlo al entrar
            if (rawBytes && rawBytes.length > 1000) {
                let debugStr = "AVISO: No encontré los títulos (TpMt, ZV, etc.) en tu archivo local.\n\n";
                debugStr += "INFORMACIÓN TÉCNICA:\n";
                debugStr += "Tamaño: " + rawBytes.length + " bytes\n";
                alert(debugStr);
            } else {
                console.warn("Archivo de Github está vacío o dañado (Tamaño: " + (rawBytes ? rawBytes.length : 0) + " bytes).");
            }
            return false;
        }
