...
window.setAdminFilter = function(filter) {
    ...
    renderGlobalView();
        ...
        function renderGlobalView() {
            ...
            // For the Proposal Table
            let tableData = {}; // { '28-Jul': { 'Arequipa': {hl, cli: Set}, ... } }
            
            // For SIC Chart
            let sicData = {}; // { '00': {hl: 0, cli: Set}, '01': ... }
            ...
            globalOrders.forEach(o => {
                ...
                const horaStr = String(o["Hora"] || ""); // Use the new Hora mapping
                
                // Process Date for table
                let displayDate = fechaStr;
                if (fechaStr.length >= 10) {
                    const parts = fechaStr.split('/');
                    if (parts.length === 3) {
                        const months = ['Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Ago', 'Sep', 'Oct', 'Nov', 'Dic'];
                        displayDate = parts[0] + '-' + months[parseInt(parts[1], 10) - 1];
                    }
                }
                
                // Process Hour for SIC
                let hour = "00";
                if (horaStr) {
                    // Extract hour from e.g. 14:30 or decimal
            ...
            const renderCapLine = (cd, act, max) => {
                const actVal = act;
                const pct = max > 0 ? ((actVal / max) * 100).toFixed(0) : 0;
                return `CD ${cd} : ${actVal.toLocaleString('en-US', {maximumFractionDigits:0})} / ${max} (${pct}%)`;
            };
            ...
            filteredOrders.forEach(o => {
                const fecha = String(o["Fecha Entrega"] || "");
                const hl = parseFloat(o["HL"]) || 0;
                const cliente = String(o["Cliente"] || "");
                if (!daily[fecha]) daily[fecha] = { hl: 0, cli: new Set() };
                daily[fecha].hl += hl;
                daily[fecha].cli.add(cliente);
            });
            const sortedDates = Object.keys(daily).sort();
            const dailyValues = sortedDates.map(d => activeMetric === 'HL' ? daily[d].hl : daily[d].cli.size);
            
            // Build SIC cumulative chart (using filtered data)
            let sicFiltered = {};
            filteredOrders.forEach(o => {
                const hl = parseFloat(o["HL"]) || 0;
                const cliente = String(o["Cliente"] || "");
                ...
                if (horaStr) {
                    const hMatch = horaStr.match(/^(\d{1,2}):/);
                    if (hMatch) hour = hMatch[1].padStart(2, '0');
                    else {
                        const floatTime = parseFloat(horaStr);
                        if (!isNaN(floatTime) && floatTime < 1) hour = Math.floor(floatTime * 24).toString().padStart(2, '0');
                        else hour = String(horaStr).substring(0, 2).padStart(2, '0');
                    }
                }
            ...
            sortedTableDates.forEach(d => {
                ...
                tableRows += `
                    <tr>
                        ...
                        <td style="border: 1px solid #000; padding: 4px 8px; text-align: center;">${td.Arequipa.hl.toLocaleString('en-US', {maximumFractionDigits:0})}</td>
                        <td style="border: 1px solid #000; padding: 4px 8px; text-align: center;">${td.Arequipa.cli.size}</td>
                        <td style="border: 1px solid #000; padding: 4px 8px; text-align: center;">${td.Tacna.hl.toLocaleString('en-US', {maximumFractionDigits:0})}</td>
                        <td style="border: 1px solid #000; padding: 4px 8px; text-align: center;">${td.Tacna.cli.size}</td>
                        <td style="border: 1px solid #000; padding: 4px 8px; text-align: center; font-weight: bold; color: red;">${td.Ilo.hl.toLocaleString('en-US', {maximumFractionDigits:0})}</td>
                        <td style="border: 1px solid #000; padding: 4px 8px; text-align: center;">${td.Ilo.cli.size}</td>
                    </tr>
                `;
            });
            ...
            const html = `
                <div class="admin-top-section">
                    <div class="admin-caps-cards">
                        <div class="admin-cap-card">
                            <h3 class="admin-cap-title">% CAPACIDAD (HL)</h3>
                            <div class="admin-cap-line">${renderCapLine('Arequipa', caps.Arequipa.hl, maxCaps.Arequipa.hl)}</div>
                            <div class="admin-cap-line">${renderCapLine('Tacna', caps.Tacna.hl, maxCaps.Tacna.hl)}</div>
                            <div class="admin-cap-line">${renderCapLine('Ilo', caps.Ilo.hl, maxCaps.Ilo.hl)}</div>
                        </div>
                        <div class="admin-cap-card">
                            <h3 class="admin-cap-title">% CAPACIDAD (Clientes)</h3>
                            <div class="admin-cap-line">${renderCapLine('Arequipa', caps.Arequipa.cli.size, maxCaps.Arequipa.cli)}</div>
                            <div class="admin-cap-line">${renderCapLine('Tacna', caps.Tacna.cli.size, maxCaps.Tacna.cli)}</div>
                            <div class="admin-cap-line">${renderCapLine('Ilo', caps.Ilo.cli.size, maxCaps.Ilo.cli)}</div>
                        </div>
                    </div>
                    
                    <div class="admin-prop-table-container">
                        <h3 class="admin-table-title" style="text-align: center; margin-bottom: 8px;">Propuesta</h3>
                        <table class="admin-prop-table" style="border-collapse: collapse; width: 100%; font-size: 0.85rem;">
                            <thead>
                                <tr>
                                    <th rowspan="2" style="background-color: #CC0000; color: white; border: 1px solid #000; padding: 4px;">Capacidad</th>
                                    <th colspan="2" style="background-color: #CC0000; color: white; border: 1px solid #000; padding: 4px;">CD Arequipa</th>
                                    <th colspan="2" style="background-color: #CC0000; color: white; border: 1px solid #000; padding: 4px;">CD Tacna</th>
                                    <th colspan="2" style="background-color: #CC0000; color: white; border: 1px solid #000; padding: 4px;">CD Ilo</th>
                                </tr>
                                <tr>
                                    <th style="background-color: #ADD8E6; border: 1px solid #000; padding: 4px; font-weight: bold; color:#000">${maxCaps.Arequipa.hl}</th>
                                    <th style="background-color: #ADD8E6; border: 1px solid #000; padding: 4px; font-weight: bold; color:#000">${maxCaps.Arequipa.cli}</th>
                                    <th style="background-color: #ADD8E6; border: 1px solid #000; padding: 4px; font-weight: bold; color:#000">${maxCaps.Tacna.hl}</th>
                                    <th style="background-color: #ADD8E6; border: 1px solid #000; padding: 4px; font-weight: bold; color:#000">${maxCaps.Tacna.cli}</th>
                                    <th style="background-color: #ADD8E6; border: 1px solid #000; padding: 4px; font-weight: bold; color:#000">${maxCaps.Ilo.hl}</th>
                                    <th style="background-color: #ADD8E6; border: 1px solid #000; padding: 4px; font-weight: bold; color:#000">${maxCaps.Ilo.cli}</th>
                                </tr>
                                <tr>
                                    <th style="border: 1px solid #000; padding: 4px; background: #fff;"></th>
                                    <th style="background-color: #CC0000; color: white; border: 1px solid #000; padding: 4px;">HL</th>
                                    <th style="background-color: #CC0000; color: white; border: 1px solid #000; padding: 4px;">Contactos</th>
                                    <th style="background-color: #CC0000; color: white; border: 1px solid #000; padding: 4px;">HL</th>
                                    <th style="background-color: #CC0000; color: white; border: 1px solid #000; padding: 4px;">Contactos</th>
                                    <th style="background-color: #CC0000; color: white; border: 1px solid #000; padding: 4px;">HL</th>
                                    <th style="background-color: #CC0000; color: white; border: 1px solid #000; padding: 4px;">Contactos</th>
                ...
                <div class="admin-filters-section">
                    <div class="admin-cd-filters">
                        <button class="g-filter-btn ${activeGFilter === 'Todas' ? 'active black' : ''}" data-cd="Todas">Gerencia</button>
                        <button class="g-filter-btn ${activeGFilter === 'Arequipa' ? 'active' : ''}" data-cd="Arequipa">CD Arequipa</button>
                        <button class="g-filter-btn ${activeGFilter === 'Tacna' ? 'active' : ''}" data-cd="Tacna">CD tacna</button>
                        <button class="g-filter-btn ${activeGFilter === 'Ilo' ? 'active' : ''}" data-cd="Ilo">CD Ilo</button>
                        
                        <div class="sim-container" style="${activeGFilter === 'Todas' ? 'display: none;' : 'display: flex;'} align-items: center; gap: 0.5rem; margin-left: 1rem;">
                            <input type="number" id="sim-cap-hl" placeholder="Sim. HL" style="width: 80px; padding: 0.3rem;" value="${activeGFilter !== 'Todas' ? maxCaps[activeGFilter].hl : ''}">
                            <input type="number" id="sim-cap-cli" placeholder="Sim. Cli" style="width: 80px; padding: 0.3rem;" value="${activeGFilter !== 'Todas' ? maxCaps[activeGFilter].cli : ''}">
                            <button id="btn-simulate" style="padding: 0.3rem 0.6rem; cursor:pointer;">Simular</button>
                        </div>
                    </div>
                    <div class="admin-metric-filters">
                        <button class="m-filter-btn ${activeMetric === 'HL' ? 'active' : ''}" data-metric="HL">HL</button>
                        <button class="m-filter-btn ${activeMetric === 'Clientes' ? 'active' : ''}" data-metric="Clientes">Clientes</button>
                    </div>
                </div>
                
                <div class="admin-charts-section">
                    <div class="admin-chart-box">
                        <h3 class="admin-chart-title">Detalle por Día</h3>
                        <div style="height: 300px; width: 100%;">
                            <canvas id="dailyChart"></canvas>
                        </div>
                    </div>
                    <div class="admin-chart-box">
                        <h3 class="admin-chart-title">SIC de Preventa</h3>
                        <div style="height: 300px; width: 100%; position: relative;">
                            <div class="sic-zones">
                                <div class="sic-zone sic-red"></div>
                                <div class="sic-zone sic-yellow"></div>
                                <div class="sic-zone sic-green"></div>
                            </div>
                            <canvas id="sicChart" style="position: absolute; top:0; left:0; z-index: 10;"></canvas>
                            <div style="text-align: center; font-weight: 500; font-size: 0.9rem; margin-top: 5px;">Avance por horas</div>
                            <div style="position: absolute; left: -20px; top: 150px; transform: rotate(-90deg); font-size: 0.8rem; color: #666;">Dias Pasados</div>
                        </div>
                    </div>
                </div>
            `;
                ...
                new Chart(ctxDaily, {
                    ...
                    data: {
                        ...
                        datasets: [{
                            label: activeMetric,
                            data: dailyValues,
                            backgroundColor: ['#10B981', '#EF4444', '#EF4444', '#F59E0B', '#10B981', '#10B981'],
                            borderRadius: 4,
                            barThickness: 30
                        }]
                    },
            ...
            if (ctxSic && window.Chart) {
                const maxVal = Math.max(...sicCumulative, 20); // Dynamic max value based on data
                    ...
                    options: {
                        ...
                        plugins: {
                            ...
                            datalabels: {
                                ...
                                formatter: (val) => (val > 0 ? (val/1000).toFixed(1) : '')
                            }
                        },
                        scales: {
                            y: { 
