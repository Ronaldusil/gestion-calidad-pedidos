    ...
    renderGlobalView();
        ...
        function renderGlobalView() {
            ...
            if (window.globalViewCache) {
                ...
                let globalDaysToAdd = 1;
                if (globalToday.getDay() === 6) globalDaysToAdd = 2;
                globalToday.setDate(globalToday.getDate() + globalDaysToAdd);
                const globalTargetDateStr = window.esPeFormatter ? window.esPeFormatter.format(globalToday) : globalToday.toLocaleDateString('es-PE', { day: '2-digit', month: 'short' });
            ...
            const targetDateStrSic = globalTargetDateStrSic;

            const sicToday = new Date();
            let sicDaysToAdd = 1;
            if (sicToday.getDay() === 6) sicDaysToAdd = 2;
            sicToday.setDate(sicToday.getDate() + sicDaysToAdd);
            const globalTargetDateStrSic = window.esPeFormatter ? window.esPeFormatter.format(sicToday) : sicToday.toLocaleDateString('es-PE', { day: '2-digit', month: 'short' });
            ...
            Object.keys(sicFiltered).forEach(h => {
                if (parseInt(h) < 7) {
                    cumValue += (activeMetric === 'HL' ? sicFiltered[h].hl : sicFiltered[h].cli.size);
                }
            });
            ...
            const html = `
                ...
                <div class="admin-top-section" style="display: flex; flex-wrap: wrap; gap: 20px; margin-bottom: 24px;">
                    ...
                    <div class="card" style="flex: 1 1 23%; min-width: 250px;">
                        ${renderCapHTML('% CAPACIDAD (Cli.)', {Arequipa:caps.Arequipa.cli.size, Tacna:caps.Tacna.cli.size, Ilo:caps.Ilo.cli.size}, {Arequipa:maxCaps.Arequipa.cli, Tacna:maxCaps.Tacna.cli, Ilo:maxCaps.Ilo.cli})}
                    </div>
