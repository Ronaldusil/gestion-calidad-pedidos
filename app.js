document.addEventListener('DOMContentLoaded', () => {
    ...
    if(deliveryFilter) {
        ...
        deliveryFilter.addEventListener('change', triggerFilterUpdate);
    }
    
    if(clientSearch) {
        clientSearch.addEventListener('input', triggerFilterUpdate);
    }
    ...
    function renderDashboard(key, role) {
        ...
        if (uniqueBlockedOrders > 0) {
            statOrdersSub.innerHTML = `Se tiene <span style="font-weight:700;">${uniqueBlockedOrders}</span> para gestionar liberación!`;
        ...
        if (prepagoClients.size > 0) {
            clientSubHtml += `<div style="color: #374151; margin-bottom: 2px;">Se tiene <span style="font-weight:700; color:#1D4ED8;">${prepagoClients.size}</span> para solicitar voucher.</div>`;
        }
        if (rejectedClients.size > 0) {
            clientSubHtml += `<div style="color: #B91C1C;">Se tiene <span style="font-weight:700;">${rejectedClients.size}</span> para reforzar recepción de pedido.</div>`;
        }
