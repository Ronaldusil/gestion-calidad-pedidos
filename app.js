document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderDashboard(key, role) {
        ...
        if (statBlocked) {
            statBlocked.textContent = blockedClients.size;
            statBlocked.style.color = blockedClients.size > 0 ? '#F59E0B' : 'inherit';
        }
        
        const statRejected = document.getElementById('stat-rejected');
        if (statRejected) {
            statRejected.textContent = rejectedClients.size;
            statRejected.style.color = rejectedClients.size > 0 ? '#EF4444' : 'inherit';
        }
        
        if (statSales) {
            statSales.innerHTML = `<span style="font-size: 1.15rem;"> ${cajasTotal.toLocaleString('en-US', { maximumFractionDigits: 2 })} CA<br> ${hlTotal.toLocaleString('en-US', { maximumFractionDigits: 2 })} HL</span>`;
        }
...
window.setAdminFilter = function(filter) {
                ...
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2rem; flex-wrap: wrap; gap: 1.5rem;">
                    ...
                    <div style="display: flex; gap: 0.5rem; background: var(--bg-secondary); padding: 0.5rem; border-radius: 12px; border: 1px solid var(--glass-border);">
                        ${['Todas', 'Arequipa', 'Tacna', 'Ilo'].map(f => `
                            <button class="g-filter-btn ${activeFilter === f ? 'active' : ''}" data-cd="${f}">${f}</button>
                        `).join('')}
                    </div>
                </div>
                ...
                <div class="glass-panel" style="padding: 2rem; border-radius: 16px;">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem;">
                        <div style="display: flex; align-items: center; gap: 0.5rem;">
                            ...
                            <h3 style="font-size: 1.2rem; font-weight: 700; margin: 0; color: var(--text-primary);">Proyeccion de Carga (HL) - ${activeFilter}</h3>
                        </div>
    ...
    function formatToIsoDate(excelDate) {
            ...
            if (activeFilter === 'Todas') {
                ...
                const exactKey = cdKeys.find(k => k.toUpperCase() === cdKey.toUpperCase());
            ...
            globalOrders.forEach(o => {
                ...
                if (!evolution[dateStr]) {
                    evolution[dateStr] = { count: 0, hl: 0, cajas: 0, bloqueados: 0, rechazos: 0, pedidos: new Set() };
                }
                evolution[dateStr].count++;
                evolution[dateStr].hl += (parseFloat(o["HL"]) || 0);
                evolution[dateStr].cajas += (parseFloat(o["Caja"]) || 0);
                if (o["N pedido"]) evolution[dateStr].pedidos.add(o["N pedido"]);
            ...
            const sortedDates = Object.keys(evolution).sort((a, b) => {
...
picoMaxPct = pct;
            ...
            let html = `
                <div style="display: flex; justify-content: flex-end; align-items: center; margin-bottom: 3rem; flex-wrap: wrap; gap: 1.5rem;">
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 1.5rem; margin-bottom: 2rem;">
                    <div class="glass-panel" style="background: linear-gradient(135deg, #fff, #f8f9fa); padding: 1.5rem; border-radius: 16px; border-left: 4px solid ${picoMaxPct > 100 ? '#DC2626' : '#10B981'}; box-shadow: 0 4px 15px rgba(0,0,0,0.03);">
                        ...
                        <div style="font-size: 1.5rem; font-weight: 800; color: var(--text-primary);">${picoMaxPct > 100 ? '' : '[V]'} ${picoFecha}</div>
                    ...
                    <div class="glass-panel" style="background: linear-gradient(135deg, #fff, #f8f9fa); padding: 1.5rem; border-radius: 16px; border-left: 4px solid ${totalBloqueadosPeriodo > 0 ? '#DC2626' : '#10B981'}; box-shadow: 0 4px 15px rgba(0,0,0,0.03);">
                        <div style="color: var(--text-secondary); font-size: 0.9rem; font-weight: 600; text-transform: uppercase; margin-bottom: 0.5rem;">Bloqueados (Credito)</div>
                        <div style="font-size: 1.5rem; font-weight: 800; color: var(--text-primary);"> ${totalBloqueadosPeriodo}</div>
                        <div style="font-size: 0.9rem; color: var(--text-secondary); margin-top: 0.25rem;">Pedidos en los proximos 7 dias</div>
                    </div>
                    ...
                    <div class="glass-panel" style="background: linear-gradient(135deg, #fff, #f8f9fa); padding: 1.5rem; border-radius: 16px; border-left: 4px solid ${totalRechazosPeriodo > 0 ? '#F59E0B' : '#10B981'}; box-shadow: 0 4px 15px rgba(0,0,0,0.03);">
                        ...
                        <div style="font-size: 1.5rem; font-weight: 800; color: var(--text-primary);"> ${totalRechazosPeriodo}</div>
                        <div style="font-size: 0.9rem; color: var(--text-secondary); margin-top: 0.25rem;">Casos en los proximos 7 dias</div>
                    </div>
                </div>
                ...
                <div class="glass-panel" style="background: var(--bg-secondary); border: 1px solid var(--glass-border); border-radius: 16px; overflow: hidden; box-shadow: 0 10px 30px rgba(0,0,0,0.05); margin-bottom: 2rem; padding: 1.5rem;">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem;">
                        <h3 style="margin: 0; color: var(--text-primary); font-size: 1.2rem;"><i class="ph ph-trend-up"></i> Proyeccion de Carga (HL) - ${activeFilter}</h3>
                        ...
                        <table style="width: 100%; border-collapse: collapse; text-align: left;">
                            <thead>
                                <tr style="background: rgba(0,0,0,0.02); border-bottom: 2px solid var(--glass-border);">
                                    ...
                                    <th style="padding: 1.2rem; color: var(--text-secondary); font-weight: 700; text-transform: uppercase; font-size: 0.85rem; letter-spacing: 1px; text-align: center;">Bloqueados (Credito)</th>
            ...
            if (activeFilter === 'Todas') {
                ...
                cdKeys.forEach(cd => {
                    ...
                    const alertIcon = pct > 100 ? '' : (pct > 80 ? '' : '[V]');
                    ...
                    bubblesHtml += `
                        <div onclick="window.setAdminFilter('${cd.replace('CD ', '')}')" class="glass-panel" style="cursor: pointer; padding: 1.5rem 2rem; border-radius: 20px; display: flex; flex-direction: column; align-items: flex-start; gap: 0.5rem; transition: transform 0.2s, box-shadow 0.2s; background: linear-gradient(135deg, rgba(255,255,255,0.95), rgba(255,255,255,0.85)); box-shadow: 0 8px 32px rgba(0,0,0,0.05); min-width: 320px;" onmouseover="this.style.transform='translateY(-5px)'; this.style.boxShadow='0 12px 40px rgba(255, 87, 34, 0.15)';" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='0 8px 32px rgba(0,0,0,0.05)';">
                            ...
                            <div style="display: flex; flex-direction: column; gap: 0.5rem; width: 100%; margin-top: 0.5rem;">
                                ...
                                <div style="font-size: 0.95rem; color: var(--text-secondary); display: flex; justify-content: space-between;">
                                    <span>Bloqueos de Credito</span>
                                    <span style="font-weight: bold; color: ${bloqueados > 0 ? '#DC2626' : 'var(--text-primary)'}">${bloqueados > 0 ? '' : '[V]'} ${bloqueados}</span>
                                </div>
                                <div style="font-size: 0.95rem; color: var(--text-secondary); display: flex; justify-content: space-between;">
                                    <span>Rechazos / Reprog.</span>
                                    <span style="font-weight: bold; color: ${rechazos > 0 ? '#DC2626' : 'var(--text-primary)'}">${rechazos > 0 ? '' : '[V]'} ${rechazos}</span>
                                </div>
                            </div>
                        </div>
                    `;
            ...
            // APLICAR HTML PRIMERO (PARA QUE LA PAGINA NUNCA QUEDE VACIA)
            container.innerHTML = html;

            // LUEGO INICIALIZAR EL GRAFICO CON UN TRY-CATCH INDEPENDIENTE
            try {
                                ...
                                datasets: [
                                    {
                                        label: 'Proyeccion HL',
                                        data: chartData,
                                        backgroundColor: chartData.map(val => val > targetHLCapacity ? 'rgba(220, 38, 38, 0.8)' : 'rgba(255, 153, 0, 0.8)'),
                                        borderRadius: 6,
                                        borderWidth: 0
                                    },
                                    {
                ...
                btn.addEventListener('click', (e) => {
                    ...
                    console.warn("Chart.js no esta cargado. El grafico no se mostrara, pero la tabla si.");
                }
            } catch (chartErr) {
                console.error("Error al renderizar el grafico Chart.js:", chartErr);
            }
        } catch (e) {
            console.error("Error fatal en renderGlobalView:", e);
            container.innerHTML = `<div style="padding: 2rem; color: #DC2626; background: #FEE2E2; border-radius: 8px;">
                <h3>Oops! Ocurrio un error al cargar la vista global.</h3>
                <p>${e.message}</p>
            </div>`;
        }
    }
