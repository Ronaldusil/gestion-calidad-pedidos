document.addEventListener('DOMContentLoaded', () => {
    ...
    function startPrefetch() {
        // Cache buster que cambia cada 5 minutos (300,000 ms). 
        // Esto permite cargar súper rápido si el usuario refresca la página
        ...
        prefetchPromise = fetch(excelUrl + cacheBuster)
            .then(res => {
                if (!res.ok) return fetch(excelUrlUpper + cacheBuster);
                return res;
            })
            .then(res => {
    ...
    async function loadDataAutomatically() {
        ...
        try {
            ...
            if (!data) {
                ...
                let res = await fetch('https://raw.githubusercontent.com/Ronaldusil/gestion-calidad-pedidos/main/pedidos.xlsx' + cacheBuster);
                if (!res.ok) res = await fetch('https://raw.githubusercontent.com/Ronaldusil/gestion-calidad-pedidos/main/pedidos.XLSX' + cacheBuster);
    ...
    function renderDashboard(key, role) {
        ...
        if (filterDateStr || searchStr) {
            orders = orders.filter(order => {
                ...
                if (searchStr) {
                    ...
                    const clientId = String(order["Cliente"] || "").toLowerCase();
        ...
        const uniqueBlocked = new Set(bloqueadosOrders.map(o => o["N pedido"])).size;
        ...
        const uniqueClients = new Set(orders.map(o => o["Cliente"])).size;
        ...
        statBoxes.innerHTML = `
            ${cajasTotal.toLocaleString('en-US', { maximumFractionDigits: 2 })} CA<br>
            <span style="color:var(--accent-primary); font-size:0.9em;">${hlTotal.toLocaleString('en-US', { maximumFractionDigits: 2 })} HL</span>
        `;
        ...
        orders.forEach((order, index) => {
            const clientName = order["Nombre 1"] || order["Nombre1"] || "Sin Nombre";
            const clientId = order["Cliente"] || "N/A";
            const material = order["Material"] || "N/A";
            const desc = order["Denominación"] || "N/A";
            
            const dateCreated = formatDate(order["Creado el"]);
            const dateDelivery = formatDate(order["Fecha Entrega"]);
            ...
            tr.innerHTML = `
                ...
                <td>
                    <strong>${qty}</strong> <span style="font-size: 0.8em; color: #888;">${um}</span>
                </td>
                <td>
                    <div class="date-badge" style="background: rgba(168, 85, 247, 0.2); color: #c084fc;">${dateDelivery}</div>
                </td>
                <td>
