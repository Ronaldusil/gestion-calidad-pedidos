document.addEventListener('DOMContentLoaded', () => {
    ...
    function processExcelData(workbook, rawBytes) {
        ...
        if (sellerKeys.length > 0) {
            renderDashboard(sellerKeys[0]); // Mostrar el primero por defecto
        } else {
            renderDashboard();
        }
    ...
    function renderDashboard(zv) {
        ...
        if (!filterDateStr) {
            ...
            tr.innerHTML = '<td colspan="9" style="text-align:center; padding: 2rem; color: #6b7280;">Seleccione una fecha para ver el detalle de pedidos</td>';
