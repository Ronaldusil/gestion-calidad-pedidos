    ...
    function renderDashboard(key, role) {
        ...
        if (statCategories) {
            ...
            Object.keys(categoryStats).sort((a,b) => categoryStats[b].cajas - categoryStats[a].cajas).forEach(cat => {
                catHtml += `<div style="display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid #f3f4f6; padding-bottom:6px;">
                    <span style="font-size:0.9rem; color:#495057;">${cat}</span>
                    <strong style="font-size:0.95rem;">${categoryStats[cat].cajas.toLocaleString('en-US', {maximumFractionDigits:0})} <span style="font-size:0.7em;color:var(--success);">CA</span></strong>
                </div>`;
            });
            statCategories.innerHTML = catHtml || '<div style="font-size:0.85rem; color:#6c757d;">No hay datos</div>';
        }
