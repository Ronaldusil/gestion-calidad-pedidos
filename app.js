...
function renderGlobalView() {
    ...
    const globalUniqueOrders = new Set(globalOrders.map(o => o["Nº pedido cliente"])).size;
    const globalBloqueados = globalOrders.filter(o => String(o["AP"] || "").trim().toUpperCase() === "C");
    const globalUniqueBlocked = new Set(globalBloqueados.map(o => o["Nº pedido cliente"])).size;
    
    let globalTotalCajas = 0;
    globalOrders.forEach(o => {
        const qty = parseFloat(o["Cantidad de pedido"]) || 0;
        globalTotalCajas += qty;
    });
    ...
    let html = `
        ...
        <div class="stat-card glass-panel" style="flex-direction: column; align-items: flex-start; gap: 1rem; margin-bottom: 2rem; background: var(--accent-primary); border: none;">
            <div style="display: flex; align-items: center; gap: 1rem; width: 100%; border-bottom: 1px solid rgba(255,255,255,0.2); padding-bottom: 1rem;">
                <div class="stat-icon-wrapper" style="background: rgba(255,255,255,0.2); color: #fff;">
                    <i class="ph ph-globe-hemisphere-west"></i>
                </div>
                <h3 style="font-size: 1.8rem; color: #fff; margin: 0;">Total Gerencia</h3>
            </div>
            
            <div style="width: 100%; display: flex; justify-content: space-between; margin-top: 0.5rem;">
                <span style="color: rgba(255,255,255,0.9); font-size: 1.1rem;">Pedidos Totales:</span>
                <strong style="color: #fff; font-size: 1.5rem;">${globalUniqueOrders}</strong>
            </div>
            
            <div style="width: 100%; display: flex; justify-content: space-between;">
                <span style="color: rgba(255,255,255,0.9); font-size: 1.1rem;">Volumen (Cajas/MKP):</span>
                <strong style="color: #fff; font-size: 1.5rem;">${globalTotalCajas.toLocaleString('en-US', { maximumFractionDigits: 2 })}</strong>
            </div>
            
            <div style="width: 100%; display: flex; justify-content: space-between;">
                <span style="color: rgba(255,255,255,0.9); font-size: 1.1rem;">Pedidos Bloqueados:</span>
                <strong style="color: #fff; font-size: 1.5rem;">${globalUniqueBlocked}</strong>
            </div>
        </div>
